...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.f, 1.f, 1.f, .3f), 9.f, 0, 1.f);
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(120 / 255.0f, 29 / 255.0f, 77 / 255.0f, it_anim->second.animation), false, 9.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(1.f, 1.f, 1.f, .1f), 3.0f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(1.0f, 1.0f, 1.0f, 0.03f), 3.0f);
    draw->AddRect(clickable.Min, clickable.Max, ImColor(66 / 255.f, 66 / 255.f, 66 / 255.f, 0.6f), 3.f, 0, 1.f);
    ...
    PushStyleColor(ImGuiCol_Border, ImVec4(66 / 255.f, 66 / 255.f, 66 / 255.f, 0.6f));
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(30, 25, 31)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x + 13, frame_bb.Max.y), ImColor(15, 15, 16), 5.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 13, frame_bb.Max.y), ImColor(120 / 255.0f, 29 / 255.0f, 77 / 255.0f), 5.0f);
    draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(66 / 255.f, 66 / 255.f, 66 / 255.f, 0.6f), 5.f, 0, 1.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 13, grab_bb.Min.y + 1), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        draw_window = g.CurrentWindow; // Child window
        draw_window->DC.NavLayersActiveMaskNext |= draw_window->DC.NavLayersActiveMask; // This is to ensure that EndChild() will display a navigation highlight
        inner_size.x -= draw_window->ScrollbarSizes.x;
    }
    else
    {
    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        if (is_multiline || (flags & ImGuiInputTextFlags_CallbackHistory))
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Up) | (1 << ImGuiDir_Down);
        g.ActiveIdUsingNavInputMask |= (1 << ImGuiNavInput_Cancel);
        g.ActiveIdUsingKeyInputMask2 |= ((ImU64)1 << ImGuiKey_Home) | ((ImU64)1 << ImGuiKey_End);
        if (is_multiline)
            g.ActiveIdUsingKeyInputMask2 |= ((ImU64)1 << ImGuiKey_PageUp) | ((ImU64)1 << ImGuiKey_PageDown); // FIXME-NAV: Page up/down actually not supported yet by widget, but claim them ahead.
        if (flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_AllowTabInput))  // Disable keyboard tabbing out as we will use the \t character.
            g.ActiveIdUsingKeyInputMask2 |= ((ImU64)1 << ImGuiKey_Tab);
    }
